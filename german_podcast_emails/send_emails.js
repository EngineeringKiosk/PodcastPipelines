...
const CC_EMAILS = ["sarah.wagner@inwt-statistics.de", "andygrunwald@gmail.com"];
...
if (!EMAIL_FILE) {
  console.error(
    "❌ No email file specified. Use --file=<filename> to specify the CSV file.",
  );
  process.exit(1);
}
...
// Email template
const EMAIL_SUBJECT =
  "{{podcast-name}} in der ersten großen deutschsprachigen Tech-Podcast-Umfrage";
...
console.log(
  `CC Emails: ${CC_EMAILS.length > 0 ? CC_EMAILS.join(", ") : "None"}`,
);
...
async function sendEmail(to, podcastName, authorName, welcome) {
  const personalizedText = EMAIL_TEMPLATE.replace(
    "{{welcome}}",
    welcome || "Hallo,",
  )
    .replace("{{email}}", to)
  ...
  const personalizedSubject = EMAIL_SUBJECT.replace(
    "{{podcast-name}}",
    podcastName,
  );
  ...
  try {
    ...
    console.error(
      `❌ Failed to send email to ${to} (${podcastName}):`,
      error.message,
    );
    return false;
  }
...
async function main() {
  try {
    ...
    for (const podcast of data) {
      ...
      const emails = podcast.Emails.split(" ").filter(
        (email) => email.trim() !== "",
      );
      ...
      for (const email of emails) {
        ...
        if (processedEmails.has(emailAddress)) {
          console.log(
            `⚠️ Skipping duplicate email ${email.trim()} (${podcast["Podcast Name"]})`,
          );
          duplicateCount++;
    ...
    console.log(
      `   📧 Duplicate emails: ${Array.from(duplicateEmails).join(", ")}`,
    );
  } catch (error) {
